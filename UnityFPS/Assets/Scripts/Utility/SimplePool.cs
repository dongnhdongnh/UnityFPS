...
public static class SimplePool
{
	...
	class Pool
	{
		...
		public GameObject Spawn(Vector3 pos, Quaternion rot)
		{
			...
			else
			{
				...
				obj = inactive.Pop();
				//	Debug.LogError("OP: Im get " + obj.gameObject.name);
				if (obj == null)
				{
	...
	static public GameObject Spawn(GameObject prefab, Vector3 pos, Quaternion rot, Transform parent)
	{
		...
		GameObject thing = pools[prefab].Spawn(pos, rot);
		thing.transform.SetParent(parent);
		return thing;
	}
	...
	/// </summary>
	static public void Despawn(GameObject obj, bool resetParent = true)
	{
		PoolMember pm = obj.GetComponent<PoolMember>();
		if (pm == null)
		{
			Debug.LogError("Object '" + obj.name + "' wasn't spawned from a pool. Destroying it instead.");
