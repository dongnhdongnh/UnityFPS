...
public class PlayerPrefsX
{
	...
	public static bool SetBool(String name, bool value)
	{
		try
		{
			PlayerPrefsOnline.SetInt(name, value ? 1 : 0);
		}
		catch
		{
			return false;
	...
	{
		return PlayerPrefsOnline.GetInt(name) == 1;
	}

	public static bool GetBool(String name, bool defaultValue)
	{
		return (1 == PlayerPrefsOnline.GetInt(name, defaultValue ? 1 : 0));
	}
	...
	public static long GetLong(string key, long defaultValue)
	{
		...
		lowBits = PlayerPrefsOnline.GetInt(key + "_lowBits", lowBits);
		highBits = PlayerPrefsOnline.GetInt(key + "_highBits", highBits);
	...
	{
		int lowBits = PlayerPrefsOnline.GetInt(key + "_lowBits");
		int highBits = PlayerPrefsOnline.GetInt(key + "_highBits");
	...
	public static void SetLong(string key, long value)
	{
		...
		PlayerPrefsOnline.SetInt(key + "_lowBits", lowBits);
		PlayerPrefsOnline.SetInt(key + "_highBits", highBits);
	}
	...
	{
		if (PlayerPrefsOnline.HasKey(key))
		{
	...
	public static bool[] GetBoolArray(String key)
	{
		if (PlayerPrefsOnline.HasKey(key))
		{
			var bytes = System.Convert.FromBase64String(PlayerPrefsOnline.GetString(key));
			if (bytes.Length < 5)
			{
	...
	public static bool SetStringArray(String key, String[] stringArray)
	{
		...
		try
		{
			PlayerPrefsOnline.SetString(key, System.Convert.ToBase64String(bytes) + "|" + String.Join("", stringArray));
		}
		catch
		{
			return false;
	...
	public static String[] GetStringArray(String key)
	{
		if (PlayerPrefsOnline.HasKey(key))
		{
			var completeString = PlayerPrefsOnline.GetString(key);
	...
	private static bool SaveBytes(String key, byte[] bytes)
	{
		try
		{
			PlayerPrefsOnline.SetString(key, System.Convert.ToBase64String(bytes));
		}
		catch
		{
			return false;
	...
	{
		long tmp = Convert.ToInt64(PlayerPrefsOnline.GetString(key, "0"));
		if (tmp == 0)
		{
	...
	{
		PlayerPrefsOnline.SetString(key, time.ToBinary().ToString());
		PlayerPrefsOnline.Save();
	}
}