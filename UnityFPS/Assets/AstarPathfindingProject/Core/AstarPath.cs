...
public class AstarPath : VersionedMonoBehaviour {
	...
	public static readonly System.Version Version = new System.Version(4, 3, 11);
	...
	public override void DrawGizmos () {
		...
		if (workItems.workItemsInProgress || isScanning) {
			...
			redrawScope = new RetainedGizmos.RedrawScope(RetainedGizmosWrapper.instance.gizmos);
			...
			for (int i = 0; i < graphs.Length; i++) {
				if (graphs[i] != null && graphs[i].drawGizmos)
					graphs[i].OnDrawGizmos(RetainedGizmosWrapper.instance.gizmos, showNavGraphs, redrawScope);
			}
			Profiler.EndSample();

			if (showNavGraphs) {
				euclideanEmbedding.OnDrawGizmos();
				if (debugMode == GraphDebugMode.HierarchicalNode) hierarchicalGraph.OnDrawGizmos(RetainedGizmosWrapper.instance.gizmos, redrawScope);
			}
		}
