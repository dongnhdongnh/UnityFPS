...
namespace Pathfinding {
	...
	public class GridGraph : NavGraph, IUpdatableGraph, ITransformedGraph
		...
		IEnumerable<Progress> ScanInternalBurst (bool async) {
			...
			// Create all nodes
			var newNodes = new GridNode[width * depth];
			var allocateNodesJob = active.AllocateNodes(newNodes, width * depth, () => new GridNode());
			var allocationMethod = async ? Allocator.Persistent : Allocator.TempJob;

			var bounds = new IntRect(0, 0, width - 1, depth - 1);

			var updateNodesJob = UpdateAreaBurst(newNodes, new Int2(width, depth), bounds, dependencyTracker, allocateNodesJob, allocationMethod, true, false);

			if (async) {
				while (!updateNodesJob.IsCompleted) {
					dependencyTracker.RunMainThreadWork();
					System.Threading.Thread.Yield();
					yield return new Progress(0.5f, "Scanning grid graph");
				}
			}
			dependencyTracker.CompleteMainThreadWork();
		...
		public struct GridGraphScanData {
			...
			public void HeightCheck (GraphCollision collision) {
				...
				var raycastCommands = dependencyTracker.NewNativeArray<RaycastCommand>(numNodes, allocationMethod, NativeArrayOptions.ClearMemory);

				heightHits = dependencyTracker.NewNativeArray<RaycastHit>(numNodes, allocationMethod, NativeArrayOptions.ClearMemory);
			...
			public void CalculateWalkabilityFromHeightData (bool useRaycastNormal, bool unwalkableWhenNoGround, float maxSlope) {
				new JobNodeWalkable {
			...
			public void Connections (float maxStepHeight, bool maxStepUsesSlope, NumNeighbours neighbours, bool cutCorners, bool use2D, bool useErodedWalkability) {
				...
				}.ScheduleBatch(bounds.Height, 20, dependencyTracker);
			}

			public void Erosion (NumNeighbours neighbours, int erodeIterations, IntRect erosionWriteMask) {
				new JobErosion {
					bounds = bounds,
			...
			struct CopyBuffersJob : IJob {
				...
				public void Execute () {
					...
					JobCopyRectangleIntoLargerBuffer<Vector3>.Copy(input.nodePositions, output.nodePositions, outputSize, outputBounds);
					JobCopyRectangleIntoLargerBuffer<float4>.Copy(input.nodeNormals, output.nodeNormals, outputSize, outputBounds);
					JobCopyRectangleIntoLargerBuffer<int>.Copy(input.nodeConnections, output.nodeConnections, outputSize, outputBounds);
					JobCopyRectangleIntoLargerBuffer<uint>.Copy(input.nodePenalties, output.nodePenalties, outputSize, outputBounds);
					JobCopyRectangleIntoLargerBuffer<int>.Copy(input.nodeTags, output.nodeTags, outputSize, outputBounds);
					JobCopyRectangleIntoLargerBuffer<bool>.Copy(input.nodeWalkable, output.nodeWalkable, outputSize, outputBounds);
					JobCopyRectangleIntoLargerBuffer<bool>.Copy(input.nodeWalkableWithErosion, output.nodeWalkableWithErosion, outputSize, outputBounds);
				}
			}

			public void ReadFromNodes (GridNode[] nodes, Int2 nodeArrayBounds, IntRect readBounds, JobHandle nodesDependsOn, NativeArray<float4> graphNodeNormals) {
				bounds = readBounds;
				numNodes = readBounds.Width * readBounds.Height;
				AllocateBuffers();
				...
				new JobCopyRectangle<float4> {
					...
					outputSize = new Int2(bounds.Width, bounds.Height),
					inputBounds = readBounds,
					outputBounds = new IntRect(0, 0, bounds.Width - 1, bounds.Height - 1),
				}.Schedule(dependencyTracker);
			}

			public GridGraphScanData ReadFromNodesAndCopy (GridNode[] nodes, Int2 nodeArrayBounds, IntRect readBounds, JobHandle nodesDependsOn, NativeArray<float4> graphNodeNormals) {
				...
				Int2 outputSize = new Int2(readBounds.Width, readBounds.Height);
				var relativeCopyBounds = this.bounds.Offset(new Int2(-readBounds.xmin, -readBounds.ymin));
				new CopyBuffersJob {
			...
			public JobHandle AssignToNodes (GridNode[] nodes, Int2 nodeArrayBounds, IntRect writeMask, uint graphIndex, JobHandle nodesDependsOn, NativeArray<float4> nodeSurfaceNormals) {
				// This is a managed type, we need to trick Unity to allow this inside of a job
				var nodesHandle = System.Runtime.InteropServices.GCHandle.Alloc(nodes);

				// Assign the data to the nodes (in parallel for performance)
				var job = new JobAssignNodeData {
					nodesHandle = nodesHandle,
				...
				var job2 = new JobCopyRectangle<float4> {
					input = nodeNormals,
					output = nodeSurfaceNormals,
					inputSize = new Int2(bounds.Width, bounds.Height),
					outputSize = nodeArrayBounds,
					inputBounds = new IntRect(0, 0, bounds.Width - 1, bounds.Height - 1),
					outputBounds = bounds,
				}.Schedule(dependencyTracker);
		...
		struct GridIndexMapper : GraphUpdateObject.INodeIndexMapper {
			...
			public IntRect iterationBounds;
			public int Count => iterationBounds.Width*iterationBounds.Height;

			public GridIndexMapper(IntRect dataBounds, IntRect iterationBounds) {
				UnityEngine.Assertions.Assert.IsTrue(dataBounds.Contains(iterationBounds));
				this.dataSize = new Int2(dataBounds.Width, dataBounds.Height);
				this.iterationBounds = iterationBounds.Offset(new Int2(-dataBounds.xmin, -dataBounds.ymin));
			}

			public int this[int index] {
				get {
					int x = index % iterationBounds.Width;
					var z = index/iterationBounds.Width;
					return (z + iterationBounds.ymin)*dataSize.x + iterationBounds.xmin + x;
				}
			}
		}
		...
		JobHandle UpdateAreaBurst (GridNode[] newNodes, Int2 nodeArrayBounds, IntRect bounds, JobDependencyTracker dependencyTracker, JobHandle nodesDependsOn, Allocator allocationMethod, bool allowRecalculateGraph, bool allowReadFromGraph, GraphUpdateObject graphUpdateObject = null) {
			...
			var fullRecalculationBounds = bounds;

			if (collision.collisionCheck && collision.type != ColliderType.Ray) fullRecalculationBounds = fullRecalculationBounds.Expand(Mathf.CeilToInt(collision.diameter * 0.5f));
			var gridBounds = new IntRect(0, 0, width - 1, depth - 1);

			// Check if there is anything to do. The bounds may not even overlap the graph
			if (!IntRect.Intersects(fullRecalculationBounds, gridBounds)) return default;

			// Due to erosion a bit more of the graph may be affected by the updates in the fullRecalculationBounds
			var writeMask = fullRecalculationBounds.Expand(erodeIterations + 1);
			...
			var readBounds = writeMask.Expand(erodeIterations + 1);

			if (!allowReadFromGraph) {
				...
				fullRecalculationBounds = readBounds;
			}

			// Clamp to the grid dimensions
			readBounds = IntRect.Intersection(readBounds, gridBounds);
			fullRecalculationBounds = IntRect.Intersection(fullRecalculationBounds, gridBounds);
			writeMask = IntRect.Intersection(writeMask, gridBounds);
			...
			if (writeMask.Width*writeMask.Height < 500) dependencyTracker.ForceLinearDependencies();
			...
			var context = new GridGraphRules.Context {
				...
				data = new GridGraphScanData {
					...
					numNodes = fullRecalculationBounds.Width * fullRecalculationBounds.Height,
					transform = transform,
			...
			if (allowRecalculateGraph) {
				...
				if (heightCheck) {
					context.data.HeightCheck(collision);
					context.data.CalculateWalkabilityFromHeightData(useRaycastNormal, collision.unwalkableWhenNoGround, maxSlope);
				} else {
				...
				if (!allowReadFromGraph) throw new System.Exception("Either allowReadFromGraph or allowRecalculateGraph needs to be true");
			...
			if (graphUpdateObject != null) {
				...
				for (int z = writeMask.ymin; z <= writeMask.ymax; z++) {
					for (int x = writeMask.xmin; x <= writeMask.xmax; x++) {
						graphUpdateObject.WillUpdateNode(nodes[z*width+x]);
					}
				}

				graphUpdateObject.ApplyJob(new GridIndexMapper(context.data.bounds, IntRect.Intersection(originalBounds, gridBounds)), new GraphUpdateObject.GraphUpdateData {
			...
			context.data.Connections(maxStepHeight, maxStepUsesSlope, neighbours, cutCorners, collision.use2D, false);
			rules.ExecuteRule(GridGraphRule.Pass.AfterConnections, context);

			if (erodeIterations > 0) {
				context.data.Erosion(neighbours, erodeIterations, writeMask);

				// After erosion is done we need to recalculate the node connections
				context.data.Connections(maxStepHeight, maxStepUsesSlope, neighbours, cutCorners, collision.use2D, true);
				rules.ExecuteRule(GridGraphRule.Pass.AfterConnections, context);
			} else {
			...
			Memory.Realloc(ref nodeSurfaceNormals, context.data.nodeNormals.Length, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);

			var assignJob = context.data.AssignToNodes(newNodes, nodeArrayBounds, writeMask, graphIndex, nodesDependsOn, nodeSurfaceNormals);
			return assignJob;
		}
		...
		public override void OnDrawGizmos (RetainedGizmos gizmos, bool drawNodes, RetainedGizmos.RedrawScope redrawScope) {
			using (var helper = GraphGizmoHelper.GetSingleFrameGizmoHelper(gizmos, active, redrawScope)) {
				...
				using (helper.builder.WithMatrix(CalculateTransform().matrix)) {
					...
					if (drawNodes && width*depth*LayerCount != nodeCount) {
						...
						using (helper.builder.WithColor(color)) {
							for (int z = 0; z < d; z++) {
								helper.builder.Line(new Vector3(0, 0, z), new Vector3(w, 0, z));
							}

							for (int x = 0; x < w; x++) {
								helper.builder.Line(new Vector3(x, 0, 0), new Vector3(x, 0, d));
							}
						}
					}
				}
			...
			if (active.showUnwalkableNodes) DrawUnwalkableNodes(gizmos, nodeSize * 0.3f);
		}
		...
		void IUpdatableGraph.UpdateArea (GraphUpdateObject o) {
			...
			if (USE_BURST_UPDATE) {
				...
				var bounds = originalRect;
				var updateNodesJob = UpdateAreaBurst(nodes, new Int2(width, depth), bounds, dependencyTracker, new JobHandle(), Allocator.TempJob, o.updatePhysics, true, o);
				System.Console.WriteLine("Completing main thread work");
				dependencyTracker.CompleteMainThreadWork();
				System.Console.WriteLine("Completing work");
				updateNodesJob.Complete();
