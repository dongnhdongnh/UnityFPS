...
namespace Pathfinding {
	...
	public class RuleTexture : GridGraphRule {
		...
		[BurstCompile]
		public struct JobTexturePosition : IJob {
			[ReadOnly]
			...
			public void Execute () {
				var offset = new float2(bounds.xmin, bounds.ymin);

				for (int z = 0; z < bounds.Height; z++) {
					for (int x = 0; x < bounds.Width; x++) {
						int2 colorPos = math.clamp((int2)math.round((new float2(x, z) + offset) * scale), int2.zero, colorDataSize - new int2(1, 1));
						int colorIndex = colorPos.y*colorDataSize.x + colorPos.x;

						int dataIndex = z*bounds.Width + x;
						...
						nodePositions[dataIndex] = graphToWorld.MultiplyPoint3x4(new Vector3((bounds.xmin + x) + 0.5f, y, (bounds.ymin + z) + 0.5f));
					}
				}
			}
		}
		...
		[BurstCompile]
		public struct JobTexturePenalty : IJob {
			[ReadOnly]
			public NativeArray<int> colorData;
